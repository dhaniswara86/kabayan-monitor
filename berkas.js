...
function statusDeadline(tanggal){
    ...
    if(hari <=7)
        return `<span class="warning">🟠 ${hari} hari</span>`;

    return `<span class="safe">🟢 Aman</span>`;
...
function aktifkanFilter(){
    ...
    const tombol=document.querySelectorAll(".filter button");

    tombol.forEach(btn=>{

        btn.onclick=function(){

            tombol.forEach(b=>b.classList.remove("active"));
...
async function loadBerkas(){
    ...
    // Admin melihat semua
    // User lain hanya melihat tahapnya
